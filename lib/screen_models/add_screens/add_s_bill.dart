...
class AddSBill extends StatefulWidget {
  @override
  _AddSBillState createState() => _AddSBillState();
}

class _AddSBillState extends State<AddSBill> {
  //-------for user type-----//
  ...
  String selected_cloud_provide_rper = '0';
  ...
  void generate_Bill() async {
    ...
    setState(() {
      for (var i = 0; i < totalselected_id.length; i++) {
        subtotal.add((double.parse(total_elementwise_exclu_rate[i].toString()) -
                double.parse(total_elementwise_exclu_tax_val[i].toString()))
            .toStringAsFixed(2));
        rateo.add('uom1');
      }
    });
    ...
    try {
      ...
      formData = FormData.fromMap({
        ...
        "_req_token": token,
        "type": "add",
        "bill_to": selected_cloud_contact_id,
        "series": seriesid.toString(),
        if (seriesid.toString() == "manual") "bill_no": billno,
        "bill_date": formateddate,
        "ship_to": selected_cloud_contact_id,
        ...
        if (additional_charges.isNotEmpty &&
            additional_charges[0]['charge_value_controller'].text.isNotEmpty)
          "freight":
              additional_charges[0]['charge_value_controller'].text.toString()
        else
          "freight": '0.0',
        if (additional_charges.isNotEmpty &&
            additional_charges[1]['charge_value_controller'].text.isNotEmpty)
          "ins":
              additional_charges[1]['charge_value_controller'].text.toString()
        else
          "ins": '0.0',
        if (additional_charges.isNotEmpty &&
            additional_charges[2]['charge_value_controller'].text.isNotEmpty)
          "pack":
              additional_charges[2]['charge_value_controller'].text.toString()
        else
          "pack": '0.0',
        if (additional_charges.isNotEmpty)
        ...
        if (double.parse(total_amount_after_additional.toString()) >
        ...
        "t_date": "",
        "form_no": "",
        if (gst_type == "exclu") "inc_st": "e",
        if (gst_type == "inclu") "inc_st": "i",
        if (gst_type == "gst-non") "inc_st": "n",
        "cess": [],
        "cess_per": []
      });

      var rsp = await gbill("/member/process", "sale.php", formData);
      ...
      if (rsp.containsKey('status')) {
        ...
        if (rsp['status'].toString() == "true") {
          showPrintedMessage(context, "Success", "Bill Generated Successfully",
              Colors.white, Colors.green, Icons.info, true, "top");
          Navigator.of(context).popUntil((route) => route.isFirst);
          Navigator.pushReplacement(
              context,
              PageTransition(
                  type: PageTransitionType.fade, child: SalesScreen()));
          ...
          showPrintedMessage(context, "Failed", "Item not added", Colors.white,
  ...
  void addParty() async {
    ...
    try {
      var rsp = await apiurl("/member/process", "customer.php", {
        "type": "add",
        if (state != null) "state": state.toString() else "state": "",
      ...
      if (rsp.containsKey('status')) {
        ...
        if (rsp['status'].toString() == "true") {
          ...
          showPrintedMessage(context, "Failed", "Already Exist", Colors.white,
              Colors.redAccent, Icons.info, true, "top");
        }
      }
    } catch (error) {
      setState(() {
        ...
        if (rsp['status'].toString() == "true") {
          ...
          setState(() {
            if (isnegative_stock_allowed == true) {
              products.add({
                'iid': int.parse(rsp['iid'].toString()),
                'cat_id': int.parse(catname.toString()),
                'cat_name': Catlist[catname!.indexOf(catname.toString())]
                        ['name']
                    .toString(),
                'item_code': create_item_code_Controller.text.toString(),
                'name': add_item_name_Controller.text.toString(),
                ...
                if (purchase_price_Controller.text.isEmpty)
                  ...
                  'pur_rate':
                      int.parse(purchase_price_Controller.text.toString()),
                if (gstpercent == null)
                  'gst': 0
                else
                ...
                if (create_item_hsn_Controller.text.isEmpty)
                  ...
                  'hsn_code':
                      int.parse(create_item_hsn_Controller.text.toString()),
                'conversion': 1,
                'rate_on': 'uom1',
                'pr_type': item_type,
                'cur_bal': 0,
        ...
        if (from == 'main list') {
          ...
          setState(() {
            itemsprod[index]['rate'] =
                producuts_rate_Controller.text.toString();
  ...
  void bottomTappedbottom(int index) {
    setState(() {
      ...
      pageControllerbottom.animateToPage(index,
          duration: Duration(milliseconds: 500), curve: Curves.ease);
    });
  }
        ...
        if (int.parse(selectedids[i].toString()) > 0) {
          if (!await FlutterContacts.requestPermission(readonly: true)) {
            ...
            final Contact? contact =
                await FlutterContacts.getContact(selectedids[i].toString());
            try {
              if (contact != []) {
  ...
  void setval_rate() {
    List aval = [];
    List rateval = [];
    //debugPrint(selected_cloud_provide_rper.toString());
    setState(() {
      for (var i = 0; i < products.length; i++) {
        if (products[i]['value'] != '0.0') {
          aval.add(double.parse(products[i]['value'].toString()));
          if (selected_cloud_provide_rper.contains('-')) {
            rateval.add((double.parse(products[i]['rate'].toString()) -
                    (double.parse(products[i]['rate'].toString()) *
                        (double.parse(selected_cloud_provide_rper
                            .toString()
                            .replaceAll('-', ''))) /
                        100)) *
                double.parse(products[i]['value'].toString()));
          } else {
            rateval.add((double.parse(products[i]['rate'].toString()) +
                    (double.parse(products[i]['rate'].toString()) *
                        (double.parse(selected_cloud_provide_rper
    ...
    setState(() {
      ...
      for (var i = 0; i < additional_charges.length; i++) {
        if (gst_type != 'gst-non') {
          ...
          if (additional_charges[i]['charge_value_controller']
              .text
              .isNotEmpty) {
            aval.add(double.parse(additional_charges[i]
                    ['charge_value_controller']
                .text
                .toString()));
          } else {
    ...
    setState(() {
      ...
      for (var i = 0; i < products.length; i++) {
        ...
        if (products[i]['value'] != '0.0') {
          ...
          if (selected_cloud_provide_rper.contains('-')) {
            total_base_rate.add((double.parse(products[i]['rate'].toString()) -
                (double.parse(products[i]['rate'].toString()) *
                    (double.parse(selected_cloud_provide_rper
                        .toString()
                        .replaceAll('-', ''))) /
                    100)));
            intrim_prod_base_rate.add(
                (double.parse(products[i]['rate'].toString()) -
                    (double.parse(products[i]['rate'].toString()) *
                        (double.parse(selected_cloud_provide_rper
                            .toString()
        ...
        */ // intrim_prod_base_rate.add((double.parse(products[i]['rate'].toString())*double.parse(products[i]['value'].toString())).toStringAsFixed(2));
          ...
          if (gst_type != 'inclu') {
            ...
            if (selected_cloud_provide_rper.contains('-')) {
              totalselected_rate.add(((double.parse(
                              products[i]['rate'].toString()) -
                          (double.parse(products[i]['rate'].toString()) *
                              (double.parse(selected_cloud_provide_rper
                                  .toString()
                                  .replaceAll('-', ''))) /
                              100)) /
                      (1 + (double.parse(products[i]['gst'].toString()) / 100)))
                  .toStringAsFixed(2));
            } else {
              totalselected_rate.add(((double.parse(
                              products[i]['rate'].toString()) +
      ...
      if (gst_type == 'inclu') {
        for (var i = 0; i < totalselected_rate.length; i++) {
          ...
          if (additional_charges.isNotEmpty) {
            for (var i = 0; i < additional_charges.length; i++) {
              additional_charges[i]['sel_gst_val'] = lastgstval[i];
            }
          }
        }
      }
      if (gst_type == 'exclu') {
        for (var i = 0; i < totalselected_rate.length; i++) {
          total_elementwise_exclu_rate.add(((double.parse(
                          totalselected_rate[i].toString()) *
                      double.parse(totalselected_val[i].toString())) +
                  double.parse(((double.parse(totalselected_tax[i].toString()) /
                              100) *
                          (double.parse(totalselected_rate[i].toString()) *
                              double.parse(totalselected_val[i].toString())))
                      .toStringAsFixed(2)))
        ...
        for (var i = 0; i < totalselected_rate.length; i++) {
          total_elementwise_exclu_rate.add(((double.parse(
                  ...
                  double.parse(((double.parse(totalselected_tax[i].toString()) /
                      ...
                      .toStringAsFixed(2)))
              .toStringAsFixed(2));
          total_elementwise_exclu_tax_val.add(((double.parse('0.0') / 100) *
                  (double.parse(totalselected_rate[i].toString()) *
                      double.parse(totalselected_val[i].toString())))
              .toStringAsFixed(2));
        }
    ...
    if (query.isNotEmpty) {
      ...
      setState(() {
        ...
        for (var i = 0; i < items.length; i++) {
          final index =
              dummySearchList.indexWhere((element) => element == items[i]);
      ...
      for (var i = 0; i < items.length; i++) {
        setState(() {
          if (searchheight < MediaQuery.of(context).size.height / 3) {
            ...
            if (partynameonitem < 500) {
              partynameonitem = partynameonitem + 50;
            }
          }
        });
      }
      ...
      if (rsp.containsKey('status')) {
        ...
        if (rsp['status'].toString() == "true") {
          setState(() {
            ...
            for (var i = 0; i < 3; i++) {
              Map a = {
                'charge_name_controller': TextEditingController(),
                'charge_value_controller': TextEditingController(),
                'val': '0.0',
                'gst_pers': gstperlist,
                'sel_gst_val': gstpercent
              };
              additional_charges.add(a);
              lastgstval.add(gstpercent);
            }
            ...
            if (additional_charges.isNotEmpty) {
              ...
              var b = aliases.indexOf('Insurance');
              var c = aliases.indexOf('Packaging');
              additional_charges[0]['charge_name_controller'].text =
                  whole_allias_array[ab]['aliase'].toString();
              additional_charges[1]['charge_name_controller'].text =
                  whole_allias_array[b]['aliase'].toString();
              additional_charges[2]['charge_name_controller'].text =
                  whole_allias_array[c]['aliase'].toString();
            }
  ...
  void getseries() async {
    ...
    try {
      var rsp = await apiurl("/member/process", "series.php", {
        "type": "view_all",
      });
      //debugPrint(rsp.keys.toString());
      //debugPrint('myseries - '+rsp.toString());
      //debugPrint(rsp['default'].toString());
      //debugPrint(rsp['default'].length.toString());
      //debugPrint(rsp['default'][0]['series_id'].toString());
      if (rsp.containsKey('status')) {
        setState(() {
          showloader = false;
        });
        ...
        if (rsp['status'].toString() == "true") {
          ...
          for (var i = 0; i < rsp['data'].length; i++) {
            setState(() {
              if (rsp['data'][i]['status'].toString() == '1') {
                series.add(rsp['data'][i]);
              }
            });
          }
          if (series.isNotEmpty) {
            setState(() {
              ...
              if (rsp['default'].length > 0) {
                seriesid = rsp['default'][0]['series_id'].toString();
              }
            });
          }
          series.add({
        ...
        if (rsp['status'].toString() == "true") {
          setState(() {
            for (var i = 0; i < rsp['data'].length; i++) {
              setState(() {
                rsp['data'][i].addAll(
                    {'value': '0.0', 'controller': TextEditingController()});
              });
            }
      ...
      setState(() {
        ...
        for (var i = 0; i < itemsprod.length; i++) {
          final index = dummySearchList
              .indexWhere((element) => element['name'] == itemsprod[i]['name']);
        ...
        ? WillPopScope(
            ...
            child: isadditem_clicked == false
                ? Scaffold(
                    ...
                    bottomNavigationBar: totalselected_id.isNotEmpty
                        ? Container(
                            height: 70,
                            width: MediaQuery.of(context).size.width,
                            color: Colors.white,
                        ...
                        : Container(height: 0),
                    body: totalselected_id.isNotEmpty
                        ? Container(
                            height: MediaQuery.of(context).size.height,
                            width: MediaQuery.of(context).size.width,
                            color: Colors.blue.withOpacity(0.1),
                            child: showpartyfields == true
                                ? ListView.builder(
                                    shrinkWrap: true,
                                              ...
                                              children: [
                                                if (index == 0)
                                                  Padding(
                                                    ...
                                                    child: Container(
                                                      ...
                                                      decoration: BoxDecoration(
                                                          color: Colors
                                                              .white, // BoxShape.circle or BoxShape.retangle
                                                          ...
                                                          Container(
                                                            ...
                                                            child: TextButton(
                                                              onPressed:
                                                                  () async {
                                                                setState(() {
                                                                  start_serial_Controller
                                                                          .text =
                                                                      billno;
                                                                  heightOfeditinvoicebottom =
                                                                      MediaQuery.of(context)
                                                                              .size
                                                                              .height -
                                                                          10;
                                                                });
                                                                _invoicecontroller = await _scaffoldKey
                                                                    .currentState!
                                                                    .showBottomSheet(
                                                                        (context) {
                                                                            ...
                                                                            children: [
                                                                              Container(
                                                                                  ...
                                                                                  child: Row(children: [
                                                                                    ...
                                                                                    Container(
                                                                                        ...
                                                                                        child: Row(
                                                                                          children: [
                                                                                            TextButton(onPressed: null, child: Text('Edit Invoice Date & Number', style: TextStyle(fontSize: 16, color: AppBarColor, fontWeight: FontWeight.w600, letterSpacing: 0))),
                                                                                          ],
                                                                                        )),
                                                                                  ])),
                                                                              Divider(),
                                                                              ...
                                                                              Padding(
                                                                                ...
                                                                                child: Container(
                                                                                  ...
                                                                                  decoration: BoxDecoration(color: Colors.white, shape: BoxShape.rectangle, borderRadius: BorderRadius.only(topRight: Radius.circular(5.0), topLeft: Radius.circular(5.0), bottomLeft: Radius.circular(5.0), bottomRight: Radius.circular(5.0))),
                                                                                  child: Padding(
                                                                                    ...
                                                                                    child: TextFormField(
                                                                                      ...
                                                                                      decoration: new InputDecoration(
                                                                                        ...
                                                                                        suffixIcon: IconButton(onPressed: () {}, icon: Icon(Icons.calendar_today_sharp, color: Colors.blue)),
                                                                              ...
                                                                              Padding(
                                                                                ...
                                                                                child: Container(
                                                                                  ...
                                                                                  child: Container(
                                                                                    height: 50,
                                                                                    width: MediaQuery.of(context).size.width,
                                                                                    decoration: BoxDecoration(color: Colors.white, shape: BoxShape.rectangle, borderRadius: BorderRadius.only(topRight: Radius.circular(5.0), topLeft: Radius.circular(5.0), bottomLeft: Radius.circular(5.0), bottomRight: Radius.circular(5.0)), border: Border.all(color: Colors.grey.withOpacity(0.3))),
                                                                                    child: Padding(
                                                                                      padding: const EdgeInsets.fromLTRB(10, 0, 10, 0),
                                                                                      child: Padding(
                                                                                          ...
                                                                                          child: ButtonTheme(
                                                                                            child: DropdownButton<String>(
                                                                                              ...
                                                                                              hint: Padding(
                                                                                                ...
                                                                                                child: Text(
                                                                                                  "Auto Series",
                                                                                                  style: GoogleFonts.poppins(color: AppBarColor, fontSize: 15, fontWeight: FontWeight.w400),
                                                                                                ),
                                                                                              ),
                                                ...
                                                if (index == 0)
                                                  GestureDetector(
                                                    ...
                                                    child: AnimatedContainer(
                                                      // height: totalselected_id.isNotEmpty?partynameonitem:searchheight!=0?partynameonitem+searchheight:partynameonitem,
                                                      duration: Duration(
                                                          milliseconds: 500),
                                                      decoration: BoxDecoration(
                                                          color: Colors
                                                                  ...
                                                                  children: [
                                                                            ...
                                                                            onChanged:
                                                                              ...
                                                                              if (v.isEmpty) {
                                                                                setState(() {
                                                                                  ...
                                                                                  selectednumber.clear();
                                                                                  contacttype.clear();
                                                                                  selected_cloud_contact_id = '0';
                                                                                  selected_cloud_provide_rper = '0';
                                                                    ...
                                                                    if (showadddetails ==
                                                                      ...
                                                                      Padding(
                                                                        ...
                                                                        child:
                                                                            AnimatedContainer(
                                                                          height: pnameController.text.isNotEmpty
                                                                              ? searchheight + 50
                                                                              : searchheight,
                                                                          width: MediaQuery.of(context)
                                                                              .size
                                                                          ...
                                                                          child:
                                                                            ...
                                                                            children: [
                                                                              AnimatedContainer(
                                                                                ...
                                                                                color: AppBarColor,
                                                                                child: pnameController.text.isNotEmpty
                                                                                    ? TextButton(
                                                                                        onPressed: () {
                                                                                          setState(() {
                                                                                            add_det_height = 350;
                                                                                        ...
                                                                                        child: Row(
                                                                                          children: [
                                                                                            ...
                                                                                            SizedBox(width: 5),
                                                                                            Text(
                                                                                              'Add details for' + ' ' + pnameController.text.toString(),
                                                                                        ...
                                                                                        itemBuilder: (BuildContext context, indexk) {
                                                                                          return GestureDetector(
                                                                                            onTap: () {
                                                                                              ...
                                                                                              if (contacttype[indexk].toString() != 'Cloud') {
                                                                                                ...
                                                                                                setState(() {
                                                                                                  ...
                                                                                                  selected_cloud_provide_rper = cloudcontact[a]['rper'].toString();
                                                                                            ...
                                                                                            child: AnimatedContainer(
                                                                                              ...
                                                                                              duration: Duration(seconds: 1),
                                                                                              decoration: BoxDecoration(color: Colors.white, // BoxShape.circle or BoxShape.retangle
                                                                                                  //color: const Color(0xFF66BB6A),
                                                                                                  boxShadow: [
                                                                                                    BoxShadow(
                                                                                                      color: Colors.black.withOpacity(0.2),
                                                                                                  ...
                                                                                                  if (items.isNotEmpty && items.indexOf(items[indexk].toString()) != -1)
                                                                                                    Container(
                                                                                                      ...
                                                                                                      child: FloatingActionButton(
                                                                                                        ...
                                                                                                        child: Center(
                                                                                                          child: Text(
                                                                                                            items[indexk].toString()[0].toString().toUpperCase(),
                                                                    ...
                                                                    Padding(
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        children: [
                                                                          Text(
                                                                              'ITEMS',
                                                                              style: TextStyle(fontSize: 16, color: AppBarColor, fontWeight: FontWeight.bold, letterSpacing: 1)),
                                                                        ],
                                                                      ),
                                                                    ),
                                                                    Padding(
                                                                                ...
                                                                                children: [
                                                                                  Container(
                                                                                    ...
                                                                                    child: RaisedButton(
                                                                                      ...
                                                                                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(00)),
                                                                                      elevation: 0,
                                                                                      color: Colors.white,
                                                                                      onPressed: () {
                                                                                        setState(() {
                                                                                  ...
                                                                                  if (isregistered == true) SizedBox(width: 5),
                                                    ...
                                                    children: [
                                                      ...
                                                      Container(
                                                          ...
                                                          child: TextButton(
                                                              onPressed: () async {
                                                                setState(() {
                                                                  heightOfModalBottomSheet =
                                                                      MediaQuery.of(context)
                                                                              .size
                                                                              .height -
                                                                          10;
                                                                  edit_discount_percent_Controller
                                                                      .clear();
                                                                  discountflat[
                                                                          index] =
                                                                      '0';
                                                                  edit_item_descrip_Controller
                                                                      .text = description[
                                                                          index]
                                                                      .toString();
                                                                  discountperct[
                                                                          index] =
                                                                      '0';
                                                                  edit_discount_amount_Controller
                                                                      .clear();
                                                                  if (gst_type !=
                                                                      'inclu') {
                                                                    //this edit condition is for exclusive value or gst-non value on clicking on edit button
                                                                    edit_discount_amount_Controller
                                                                        .clear(); //clearing discount amount input field
                                                                    edit_discount_percent_Controller
                                                                        .clear(); //clearing discount percent input field
                                                                    new_price_Controller
                                                                        .text = (double.parse(totalselected_rate[index].toString()) +
                                                                            double.parse(total_elementwise_exclu_tax_val[index]
                                                                                .toString()))
                                                                        .toStringAsFixed(
                                                                            2); //setting new price value
                                                                    edit_product_base_price =
                                                                        totalselected_rate[index]
                                                                            .toString(); //setting product base price
                                                                    edit_product_quant =
                                                                        totalselected_val[index]
                                                                            .toString(); //setting product selected quantinity
                                                                    edit_product_base_with_quant = (double.parse(totalselected_val[index].toString()) *
                                                                            double.parse(totalselected_rate[index]
                                                                                .toString()))
                                                                        .toStringAsFixed(
                                                                            2); //setting value of product = base price * quantity
                                                                    edit_quant_Controller
                                                                        .text = totalselected_val[
                                                                            index]
                                                                        .toString(); //setting product selected quantity into controller
                                                                    edit_unit_Controller
                                                                        .text = totalselected_uom[
                                                                            index]
                                                                        .toString(); //setting product unit controller
                                                                    if (gst_type !=
                                                                        'gst-non') {
                                                                      //condition if gst type is not inclu and gst_none
                                                                      edit_gst_Controller
                                                                          .text = totalselected_tax[index]
                                                                              .toString() +
                                                                          ' ' +
                                                                          '%'; //setting gst selected percentage
                                                                      edit_product_tax_val =
                                                                          total_elementwise_exclu_tax_val[index]
                                                                              .toString(); //setting selected value after gst calculation
                                                                    } else {
                                                                      edit_gst_Controller
                                                                              .text =
                                                                          '0.0' +
                                                                              ' ' +
                                                                              '%'; //setting gst_controller value to 0
                                                                      edit_product_tax_val =
                                                                          '0.0'; // setting tax value to 0
                                                                    }
                                                                  } else {
                                                                    //this edit condition is for inclusive value on clicking on edit button
                                                                    new_price_Controller
                                                                        .text = total_elementwise_exclu_rate[
                                                                            index]
                                                                        .toString(); //setting new price value
                                                                    edit_gst_Controller
                                                                        .text = totalselected_tax[index]
                                                                            .toString() +
                                                                        ' ' +
                                                                    ...
                                                                    edit_quant_Controller
                                                                        .text = totalselected_val[
                                                                            index]
                                                                        .toString(); //setting product selected quantity into controller
                                                                    edit_total_amount_inclu_case = double.parse(total_elementwise_exclu_rate[index]
                                                                            .toString())
                                                                        .toStringAsFixed(
                                                                            2);
                                                                    edit_total_tax_rate_inclu_case =
                                                                        edit_product_tax_val
                                                                            .toString();
                                                                    edit_prod_mult_quant_iclu_case =
                                                                        edit_product_base_with_quant
                                                                            .toString();
                                                                  }
                                                                });
                                                                _bottomcontroller = await _scaffoldKey
                                                                    .currentState!
                                                                    .showBottomSheet(
                                                                        (context) {
                                                                  return new Container(
                                                                            ...
                                                                            children: [
                                                                              Container(
                                                                                  ...
                                                                                  child: Row(children: [
                                                                                    ...
                                                                                    Container(
                                                                                        ...
                                                                                        child: Row(
                                                                                          children: [
                                                                                            TextButton(onPressed: null, child: Text(totalselected_name[index].toString(), style: TextStyle(fontSize: 16, color: AppBarColor, fontWeight: FontWeight.w600, letterSpacing: 0))),
                                                                                          ],
                                                                                        )),
                                                                                  ])),
                                                                              Divider(),
                                                                                            ...
                                                                                            if (indexedit == 0)
                                                                                              Padding(
                                                                                                ...
                                                                                                child: Container(
                                                                                                  height: 50,
                                                                                                  width: MediaQuery.of(context).size.width - 10,
                                                                                                  decoration: BoxDecoration(color: Colors.grey.withOpacity(0.3), shape: BoxShape.rectangle, borderRadius: BorderRadius.only(topRight: Radius.circular(5.0), topLeft: Radius.circular(5.0), bottomLeft: Radius.circular(5.0), bottomRight: Radius.circular(5.0))),
                                                                                                  child: Padding(
                                                                                                    padding: const EdgeInsets.fromLTRB(10, 0, 10, 0),
                                                                                                    child: TextFormField(
                                                                                                      readOnly: true,
                                                                                                      ...
                                                                                                      decoration: new InputDecoration(
                                                                                                        ...
                                                                                                        prefixIcon: TextButton(onPressed: null, child: Text('₹', style: TextStyle(fontSize: 30))),
                                                                                                        isDense: true, floatingLabelBehavior: FloatingLabelBehavior.never,
                                                                                                        labelText: "",
                                                                                                        fillColor: Colors.white.withOpacity(0.5),
                                                                                                        focusedBorder: OutlineInputBorder(
                                                                                                            ...
                                                                                                            decoration: new InputDecoration(
                                                                                                              ...
                                                                                                              suffixIcon: Container(
                                                                                                                  ...
                                                                                                                  child: Padding(
                                                                                                                    ...
                                                                                                                    child: Row(
                                                                                                                      children: [
                                                                                                                        if (ispercent == true) Text('%', style: TextStyle(fontSize: 15, color: Colors.black)) else Text('a', style: TextStyle(fontSize: 15, color: Colors.white)),
                                                                                                                        SizedBox(width: 10),
                                                                                                                        RaisedButton(
                                                                                                                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10.0), side: BorderSide(color: Colors.grey.withOpacity(0.3))),
                                                                                                                            highlightColor: Colors.transparent,
                                                                                                                            splashColor: Colors.transparent,
                                                                                                                            onPressed: () {
                                                                                                                              setState(() {
                                                                                                                            ...
                                                                                                                            elevation: 0,
                                                                                                                            color: Colors.white,
                                                                                                                            child: ispercent == true ? Text('Percentage') : Text('Amount')),
                                                                                                                      ],
                                                                                                                    ),
                                                                                                                  )),
                                                                                                              isDense: true, floatingLabelBehavior: FloatingLabelBehavior.never,
                                                                                                  ...
                                                                                                  child: Padding(
                                                                                                    ...
                                                                                                    child: TextFormField(
                                                                                                      ...
                                                                                                      decoration: new InputDecoration(
                                                                                                        ...
                                                                                                        prefixIcon: TextButton(onPressed: null, child: Text('GST @', style: TextStyle(fontSize: 18, color: Colors.black))),
                                                                                                        isDense: true, floatingLabelBehavior: FloatingLabelBehavior.never,
                                                                                                        labelText: "",
                                                                                                        fillColor: Colors.white.withOpacity(0.5),
                                                                                                        focusedBorder: OutlineInputBorder(
                                                                              ...
                                                                              Padding(
                                                                                ...
                                                                                child: Container(
                                                                                  ...
                                                                                  width: MediaQuery.of(context).size.width - 10,
                                                                                  decoration: BoxDecoration(color: Colors.grey.withOpacity(0.1), shape: BoxShape.rectangle, borderRadius: BorderRadius.only(topRight: Radius.circular(10.0), topLeft: Radius.circular(10.0), bottomLeft: Radius.circular(10.0), bottomRight: Radius.circular(10.0))),
                                                                                  child: Padding(
                                                                                      ...
                                                                                      child: Container(
                                                                                          ...
                                                                                          Container(
                                                                                            ...
                                                                                            child: Row(
                                                                                              ...
                                                                                              children: [
                                                                                                ...
                                                                                                if (gst_type != 'inclu') Text(edit_product_base_with_quant.toString(), style: TextStyle(fontSize: 16, color: Colors.grey)) else Text(edit_prod_mult_quant_iclu_case.toString(), style: TextStyle(fontSize: 16, color: Colors.grey)),
                                                                                              ],
                                                                                            ),
                                                                                          )
                                                                                        ]),
                                                                                        if (isregistered == true) SizedBox(height: 5),
                                                                                        if (isregistered == true)
                                                                                          Row(children: [
                                                                                            Container(
                                                                                              width: 140,
                                                                                            ...
                                                                                            Container(
                                                                                              ...
                                                                                              child: Row(
                                                                                                ...
                                                                                                children: [
                                                                                                  ...
                                                                                                  if (gst_type != 'inclu') Text(edit_product_tax_val.toString(), style: TextStyle(fontSize: 16, color: Colors.grey)) else Text(edit_total_tax_rate_inclu_case.toString(), style: TextStyle(fontSize: 16, color: Colors.grey)),
                                                                                                ],
                                                                                              ),
                                                                                            )
                                                                                          ]),
                                                                                        ...
                                                                                        Row(children: [
                                                                                          ...
                                                                                          Container(
                                                                                            ...
                                                                                            child: Row(
                                                                                              mainAxisAlignment: MainAxisAlignment.end,
                                                                                              children: [
                                                                                                if (edit_discount_percent_Controller.text.isNotEmpty) Text('(' + edit_discount_percent_Controller.text.toString() + '%' + ')', style: TextStyle(fontSize: 16, color: Colors.grey)) else Text('(' + '0.0' + '%' + ')', style: TextStyle(fontSize: 16, color: Colors.grey)),
                                                                                                Text(' ₹ ', style: TextStyle(fontSize: 16, color: Colors.grey)),
                                                                                                if (edit_discount_amount_Controller.text.isNotEmpty) Text(edit_discount_amount_Controller.text.toString(), style: TextStyle(fontSize: 16, color: Colors.grey)) else Text('0.0', style: TextStyle(fontSize: 16, color: Colors.grey)),
                                                                                              ],
                                                                                            ),
                                                                                          )
                                                                                        ]),
                                                                                        ...
                                                                                        Row(children: [
                                                                                          ...
                                                                                          Container(
                                                                                            width: MediaQuery.of(context).size.width - 190,
                                                                                            child: gst_type != 'inclu'
                                                                                                ? Row(
                                                                                                    mainAxisAlignment: MainAxisAlignment.end,
                                                                                                    children: [
                                                                                                      Text(' ₹ ', style: TextStyle(fontSize: 16, color: Colors.black, fontWeight: FontWeight.bold)),
                                                                                                      if (edit_discount_amount_Controller.text.isNotEmpty) Text((((double.parse(edit_product_base_price.toString()) * double.parse(edit_product_quant.toString())) + double.parse(edit_product_tax_val.toString())) - double.parse(edit_discount_amount_Controller.text.toString())).toStringAsFixed(2), style: TextStyle(fontSize: 16, color: Colors.black, fontWeight: FontWeight.bold)) else Text((((double.parse(edit_product_base_price.toString()) * double.parse(edit_product_quant.toString())) + double.parse(edit_product_tax_val.toString()))).toStringAsFixed(2), style: TextStyle(fontSize: 16, color: Colors.black, fontWeight: FontWeight.bold))
                                                                                                    ],
                                                                                                  )
                                                                                                : Row(
                                                                                                    mainAxisAlignment: MainAxisAlignment.end,
                                                                                        ...
                                                                                        setState(() {
                                                                                          ...
                                                                                          if (additional_charges.isNotEmpty) {
                                                                                            ...
                                                                                            for (var i = 0; i < additional_charges.length; i++) {
                                                                                              if (gst_type != 'gst-non') {
                                                                                                ...
                                                                                                if (additional_charges[i]['charge_value_controller'].text.isNotEmpty) {
                                                                                                  additional.add(double.parse(additional_charges[i]['charge_value_controller'].text.toString()));
                                                                                                } else {
                                                                                            ...
                                                                                            if (additional.isNotEmpty) {
                                                                                              ...
                                                                                              total_amount_after_additional = (double.parse(total_exclusive_val.toString()) + double.parse(total_amount_after_additional.toString())).toStringAsFixed(2);
                                                                                            } else {
                                                                                              total_amount_after_additional = '0.0';
                                                                                              //debugPrint(total_amount_after_additional.toString());
                                                                                            }
                                                                ...
                                                                Switch(
                                                                  onChanged:
                                                                    ...
                                                                    setState(
                                                                      ...
                                                                      if (v ==
                                                                          true) {
                                                                        total_exclusive_val = double.parse(total_exclusive_val.toString())
                                                                            .round()
                                                                            .toString();
                                                                        total_amount_after_additional = double.parse(total_amount_after_additional.toString())
                                                                            .round()
                                                                            .toString();
                                                                      } else {
                                                                        total_amount_after_additional =
                                                                            un_round_items_total.toString();
                                                                        total_exclusive_val =
                                                                            un_round_items_subtotal.toString();
                                                                      }
                                                                    });
                                                                  },
                                                                  value:
                                                                                ...
                                                                                child: Row(children: [
                                                                                  Padding(
                                                                                    ...
                                                                                    child: Container(
                                                                                      height: 50,
                                                                                      width: 120,
                                                                                      decoration: BoxDecoration(color: Colors.transparent, shape: BoxShape.rectangle, borderRadius: BorderRadius.only(topRight: Radius.circular(5.0), topLeft: Radius.circular(5.0), bottomLeft: Radius.circular(5.0), bottomRight: Radius.circular(5.0))),
                                                                                      child: Padding(
                                                                                        padding: const EdgeInsets.fromLTRB(00, 0, 00, 0),
                                                                                        child: TextFormField(
                                                                                          readOnly: true,
                                                                                          decoration: new InputDecoration(
                                                                                            contentPadding: EdgeInsets.only(left: 5, right: 1, top: 0, bottom: 0),
                                                                                            prefixIconConstraints: BoxConstraints(
                                                                                              minWidth: 5,
                                                                                              minHeight: 48,
                                                                                            ),
                                                                                            ...
                                                                                            isDense: true, floatingLabelBehavior: FloatingLabelBehavior.never,
                                                                                            labelText: index == 0
                                                                                                ? whole_allias_array[aliases.indexOf('Freight')]['aliase'].toString()
                                                                                                : index == 1
                                                                                                    ? whole_allias_array[aliases.indexOf('Packaging')]['aliase'].toString()
                                                                                                    : whole_allias_array[aliases.indexOf('Insurance')]['aliase'].toString(),
                                                                                        ...
                                                                                        child: TextFormField(
                                                                                          ...
                                                                                          decoration: new InputDecoration(
                                                                                            ...
                                                                                            prefixIcon: Container(
                                                                                              width: 25,
                                                                                              child: TextButton(onPressed: null, child: Text('₹', style: TextStyle(fontSize: 20, color: Colors.black, fontWeight: FontWeight.bold))),
                                                                                            ),
                                                                                            isDense: true, floatingLabelBehavior: FloatingLabelBehavior.never,
                                                                                            labelText: "0.0",
                                                                                            fillColor: Colors.white.withOpacity(0.5),
                                                                                        ...
                                                                                        child: DropdownButtonHideUnderline(
                                                                                          child: ButtonTheme(
                                                                                            child: DropdownButton<String>(
                                                                                              ...
                                                                                              style: TextStyle(color: AppBarColor),
                                                                                              iconEnabledColor: AppBarColor,
                                                                                              items: additional_charges[index]['gst_pers']
                                                                                                  .map<DropdownMenuItem<String>>((item) => new DropdownMenuItem<String>(
                                                                                                        child: new Text(
                                                                                                          item.toString() + " " + "%",
                                                                                                          style: TextStyle(fontSize: 18, color: AppBarColor),
                                                                                                        ),
                                                                                              ...
                                                                                              onChanged: double.parse(additional_charges[index]['val'].toString()) == 0 || gst_type == 'gst-non'
                                                                                                  ? null
                                                                                                  : (String? value) {
                        ...
                        : Container(
                            ...
                            width: MediaQuery.of(context).size.width,
                            color: Colors.blue.withOpacity(0.1),
                            child: is_adddetails_clicked == false
                                ? ListView(
                                    shrinkWrap: true,
                                    ...
                                    children: [
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          decoration:
                                                                            ...
                                                                            suffixIcon:
                                                                                IconButton(onPressed: () {}, icon: Icon(Icons.calendar_today_sharp, color: Colors.blue)),
                                                                            isDense:
                                                                                true,
                                                                            floatingLabelBehavior:
                                      ...
                                      if (showfirstloader == false)
                                        GestureDetector(
                                          ...
                                          child: AnimatedContainer(
                                            // height: totalselected_id.isNotEmpty?990:searchheight!=0?990+searchheight:partyname,
                                            duration:
                                                              ...
                                                              onChanged: (v) {
                                                                if (v.isEmpty) {
                                                                  setState(() {
                                                                    ...
                                                                    contacttype
                                                                        .clear();
                                                                    selected_cloud_contact_id =
                                                                        '0';
                                                                    selected_cloud_provide_rper =
                                                                        '0';
                                                                    if (totalselected_id
                                                                        .isNotEmpty) {
                                                                      partyname =
                                                              ...
                                                              children: [
                                                                ...
                                                                AnimatedContainer(
                                                                  ...
                                                                  child: items
                                                                                    ...
                                                                                    if (items.isNotEmpty && items.indexOf(items[index].toString()) != -1)
                                                                                      Container(
                                                                                        ...
                                                                                        child: FloatingActionButton(
                                                                                          ...
                                                                                          child: Center(
                                                                                            child: Text(
                                                                                              items[index].toString()[0].toString().toUpperCase(),
                                                                                              style: TextStyle(color: Colors.white),
                                                                                            ),
                                                                                          ),
                                                                                        ),
                                                                      ...
                                                                      : Center(
                                                                          child: pnameController.text.isEmpty
                                                                              ? Text('Enter name', style: TextStyle(color: Colors.black, fontSize: 15))
                                                                              : Text('Contact not available', style: TextStyle(color: Colors.black, fontSize: 15)),
                                                                        ),
                                                                ),
                                                              ],
                                                            ),
                                ...
                                : ListView(
                                    ...
                                    children: [
                                      AnimatedContainer(
                                        ...
                                        child: showaddpartyloader == false
                                            ? ListView(
                                                children: [
                                                  Row(
                                                    mainAxisAlignment:
                                                                    ...
                                                                    children: [
                                                                      Container(
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          child: Text(
                                                                              'I Receive',
                                                                              style: TextStyle(fontSize: 12, color: isrecieve == true ? Colors.white : Colors.black)),
                                                                        ),
                                                                      ),
                                                                      SizedBox(
                                                                          width:
                                                                      ...
                                                                      Container(
                                                                        ...
                                                                        child:
                                                                          ...
                                                                          child: Text(
                                                                              'I Pay',
                                                                              style: TextStyle(fontSize: 12, color: isrecieve == false ? Colors.white : Colors.black)),
                                                                        ),
                                                                      ),
                                                                    ],
                                                                  ),
                ...
                : Scaffold(
                    appBar: show_create_item_screen == false
                        ? AppBar(
                            elevation: 1,
                            backgroundColor: Colors.white,
                            centerTitle: false,
                            leading: IconButton(
                                onPressed: () {
                                  if (double.parse(totalitems.toString()) >
                                      0.0) {
                                    ...
                                    setState(() {
                                      ...
                                      if (additional_charges.isNotEmpty) {
                                        ...
                                        additional_charges[0]
                                                ['charge_name_controller']
                                            .text = whole_allias_array[ab]
                                                ['aliase']
                                            .toString();
                                        additional_charges[1]
                                                ['charge_name_controller']
                                            .text = whole_allias_array[b]
                                                ['aliase']
                                            .toString();
                                        additional_charges[2]
                                                ['charge_name_controller']
                                            .text = whole_allias_array[c]
                                                ['aliase']
                                            .toString();
                                      }
                                      discountController.text = '0.0';
                              ...
                              child: Padding(
                                ...
                                child: TextField(
                                  ...
                                  decoration: InputDecoration(
                                      labelText:
                                          "Search using name, hsn no, category",
                        ...
                        : AppBar(
                            ...
                            backgroundColor: Colors.white,
                            centerTitle: false,
                            leading: showcreateitemloader == false
                                ? IconButton(
                                    onPressed: () {
                                      setState(() {
                                        show_create_item_screen = false;
                                        ...
                                        create_item_hsn_Controller.clear();
                                        newitemgstper = null;
                                        create_item_open_stock_Controller
                                            .clear();
                                        create_item_open_amount_Controller
                                            .clear();
                                        create_item_code_Controller.clear();
                                        remarks_Controller.clear();
                                        item_type = 'Product';
...
*/
                      ...
                      child: show_create_item_screen == false
                          ? Column(
                              children: [
                                                                    ...
                                                                    Padding(
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        children: [
                                                                          Container(
                                                                            width:
                                                                                MediaQuery.of(context).size.width - 162,
                                                                            child:
                                                                                Align(
                                                                          ...
                                                                          Container(
                                                                              decoration: BoxDecoration(shape: BoxShape.rectangle, color: Colors.white, border: Border.all(color: AppBarColor), borderRadius: BorderRadius.only(topRight: Radius.circular(30.0), topLeft: Radius.circular(30.0), bottomLeft: Radius.circular(30.0), bottomRight: Radius.circular(30.0))),
                                                                              width: 110,
                                                                              child: TextFormField(
                                                                                ...
                                                                                decoration: new InputDecoration(
                                                                                  ...
                                                                                  prefixIcon: GestureDetector(
                                                                                    ...
                                                                                    child: Container(decoration: BoxDecoration(shape: BoxShape.rectangle, color: scaffoldbackground, borderRadius: BorderRadius.only(topRight: Radius.circular(0.0), topLeft: Radius.circular(30.0), bottomLeft: Radius.circular(30.0), bottomRight: Radius.circular(0.0))), height: 30, width: 10, child: Center(child: Icon(Icons.remove, color: Colors.white))),
                                                                                  ),
                                                                                  suffixIcon: GestureDetector(
                                                                                    ...
                                                                                    child: Container(decoration: BoxDecoration(shape: BoxShape.rectangle, color: scaffoldbackground, borderRadius: BorderRadius.only(topRight: Radius.circular(30.0), topLeft: Radius.circular(0.0), bottomLeft: Radius.circular(0.0), bottomRight: Radius.circular(30.0))), height: 30, width: 10, child: Center(child: Icon(Icons.add, color: Colors.white))),
                                                                                  ),
                                                                                  labelStyle: TextStyle(fontSize: 13),
                                                                                  fillColor: Colors.white.withOpacity(0.5),
                                                                                  focusedBorder: OutlineInputBorder(
                                                                          ...
                                                                          children: [
                                                                            Row(
                                                                              children: [
                                                                                ...
                                                                                Text(
                                                                                  " " + products[index]['cat_name'].toString(),
                                                                                  style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                ),
                                                                              ],
                                                                            ),
                                                                          ...
                                                                          children: [
                                                                            Row(
                                                                              children: [
                                                                                ...
                                                                                Text(
                                                                                  " " + products[index]['pr_type'].toString(),
                                                                                  style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                ),
                                                                              ],
                                                                            ),
                                                                          ...
                                                                          children: [
                                                                            Row(
                                                                              children: [
                                                                                ...
                                                                                Text(
                                                                                  " " + products[index]['cur_bal'].toString(),
                                                                                  style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                ),
                                                                              ],
                                                                            ),
                                ...
                                if (showloader == false &&
                                    itemsprod.isNotEmpty &&
                                    isbillfoundprod == true)
                                  Container(
                                    height: MediaQuery.of(context).size.height -
                                        237,
                                    child: itemsprod.isNotEmpty
                                        ? Padding(
                                            padding: const EdgeInsets.all(8.0),
                                            child: ListView.builder(
                                                shrinkWrap: true,
                                                                          ...
                                                                          children: [
                                                                            Row(
                                                                              children: [
                                                                                ...
                                                                                Text(
                                                                                  " " + itemsprod[index]['cat_name'].toString(),
                                                                                  style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                ),
                                                                              ],
                                                                            ),
                                                                          ...
                                                                          children: [
                                                                            Row(
                                                                              children: [
                                                                                ...
                                                                                Text(
                                                                                  " " + itemsprod[index]['pr_type'].toString(),
                                                                                  style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                ),
                                                                              ],
                                                                            ),
                                                                              ...
                                                                              children: [
                                                                                ...
                                                                                Row(
                                                                                  children: [
                                                                                    ...
                                                                                    //  TextField(
                                                                                    //    controller: TextEditingController(text:itemsprod[index]['rate']
                                                                                    //         .toString()),
                                                                                    //       onChanged:(value){
                                                                                    //    setState((){
                                                                                    //        itemsprod[index]['rate']=value;
                                                                                    //    });
                                                                                    //       }
                                                                                    //  ),
                                                                                    Text(
                                                                                      "" + itemsprod[index]['rate'].toString(),
                                                                                      style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                    ),
                                                                                    Text(
                                                                                      "/" + itemsprod[index]['uom1'].toString(),
                                                                                      style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                    ),
                                                                                  ],
                                                                                ),
                                                                          ...
                                                                          children: [
                                                                            Row(
                                                                              children: [
                                                                                ...
                                                                                Text(
                                                                                  " " + itemsprod[index]['value'].toString(),
                                                                                  style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                ),
                                                                              ],
                                                                            ),
                                                                          ...
                                                                          children: [
                                                                            Row(
                                                                              children: [
                                                                                ...
                                                                                Text(
                                                                                  " " + itemsprod[index]['cur_bal'].toString(),
                                                                                  style: GoogleFonts.poppins(fontSize: 13, color: Colors.black, fontWeight: FontWeight.w400),
                                                                                ),
                                                                              ],
                                                                            ),
                          ...
                          : ListView(children: [
                              ...
                              if (showcreateitemloader == false)
                                Padding(
                                  padding:
                                      const EdgeInsets.fromLTRB(0, 10, 10, 8),
                                  child: Container(
                                    height: 50,
                                    decoration: BoxDecoration(
                                        color: Colors.white,
                              ...
                              if (showcreateitemloader == false)
                                Container(
                                  ...
                                  child: Padding(
                                    padding:
                                        const EdgeInsets.fromLTRB(10, 10, 0, 8),
                                    child: Row(
                                      mainAxisAlignment:
                                                                  ...
                                                                  items: uoms.map(
                                                                        ...
                                                                        return new DropdownMenuItem(
                                                                          child:
                                                                            ...
                                                                            item['name'],
                                                                            style:
                                                                                TextStyle(fontSize: 15, color: AppBarColor),
                                                                          ),
                                                                          value:
                                                                              item['code'].toString(),
                                                                        );
                                                                        ...
                                                                        return new DropdownMenuItem(
                                                                          child:
                                                                            ...
                                                                            item['gst'] +
                                                                                ...
                                                                                "%",
                                                                            style:
                                                                                TextStyle(fontSize: 18, color: AppBarColor),
                                                                          ),
                                                                          value:
                                                                              item['gst'].toString(),
                                                                        );
                    ...
                    bottomNavigationBar: show_create_item_screen == false
                        ? Container(
                            width: MediaQuery.of(context).size.width,
                            height: 70,
                        ...
                        : Container(
                            ...
                            child: Padding(
                              ...
                              child: showcreateitemloader == false
                                  ? Container(
                                      color: Colors.indigo,
                                      child: RaisedButton(
                                          highlightColor: Colors.transparent,
