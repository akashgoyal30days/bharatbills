  ...
  void getproddet() async {
    ...
    try {
      var rsp = await apiurl(
          "/member/process", "product.php", {"type": "view", "pid": widget.id});
      ...
      if (rsp.containsKey('status')) {
        ...
        if (rsp['status'].toString() == "true") {
          setState(() {
            ...
            unitConversionController.text =
                rsp['data']['conversion'].toString();
        ...
        if (rsp['status'].toString() == "true") {
          ...
          setState(() {
            ...
            Navigator.pushReplacement(
                context,
                PageTransition(
                    type: PageTransitionType.fade, child: ProductScreen()));
          });
        } else if (rsp['status'].toString() == "false") {
          setState(() {
            showloader = false;
          });
          showPrintedMessage(context, "Failed", "Failed to update product",
              Colors.white, Colors.redAccent, Icons.info, true, "bottom");
          if (rsp['error'].toString() == "invalid_auth") {
            Navigator.of(context).popUntil((route) => route.isFirst);
            showPrintedMessage(context, "Error", "Session expired",
          ...
          showPrintedMessage(context, "Failed", "Product already exist",
              Colors.white, Colors.redAccent, Icons.info, true, "bottom");
        }
      }
    } catch (error) {
