...
class _EditProductState extends State<EditProduct> {
    ...
    try{
      ...
      if(rsp.containsKey('status')){
        ...
        if(rsp['status'].toString()=="true"){
          setState(() {
            //required controllers
            itemController.text = rsp['data']['name'].toString();
            catname = rsp['data']['cat_id'].toString();
            itemCodeController.text = rsp['data']['item_code'].toString();
            descriptionController.text = rsp['data']['item_desc'].toString();
            gstpercent = rsp['data']['gst'].toString();
            hsnController.text = rsp['data']['hsn_code'].toString();

            //unit Controllers
            uom1 = rsp['data']['uom1'].toString();
            uom2 = rsp['data']['uom2'].toString();
            unitConversionController.text = rsp['data']['conversion'].toString();
            if(rsp['data']['rate_on'].toString()=="uom2")
            _urate = UomRate.uom2rate;
            if(rsp['data']['rate_on'].toString()=="uom1")
              _urate = UomRate.uom1rate;


            //rates controllers
            salerateController.text = rsp['data']['rate'].toString();
            purchaserateController.text = rsp['data']['pur_rate'].toString();


            //level controllers
            if(rsp['data']['min_level'].toString()!='null') {
              minlevelController.text = rsp['data']['min_level'].toString();
            }
            if(rsp['data']['max_level'].toString()!='null') {
              maxlevelController.text = rsp['data']['max_level'].toString();
            }
            if(rsp['data']['re_level'].toString()!='null') {
              reorderlevelController.text = rsp['data']['re_level'].toString();
            }

            //other controllers
            openquantityController.text = rsp['data1']['open_quant'].toString();
            openamountController.text = rsp['data1']['open_amt'].toString();
            type = rsp['data']['pr_type'].toString();
          }
          );
  ...
  void add () async{
    ...
    try{
      var rsp = await apiurl("/member/process", "product.php", {
        "type":"update",
        "iid": widget.id.toString(),
        "par_val": catname.toString(),
      ...
      if(rsp.containsKey('status')){
        ...
        showPrintedMessage(context, "Failed", "Failed to update product", Colors.white,Colors.redAccent, Icons.info, true, "bottom");
        if(rsp['error'].toString()=="invalid_auth"){
            Navigator.of(context).popUntil((route) => route.isFirst);
            showPrintedMessage(context, "Error", "Session expired", Colors.white,Colors.redAccent, Icons.info, true, "bottom");
            Navigator.pushReplacement(context, PageTransition(type: PageTransitionType.fade, child: MyHomePage()));
          ...
          showPrintedMessage(context, "Failed", "Product already exist", Colors.white,Colors.redAccent, Icons.info, true, "bottom");
        }
      }
    }catch(error){
      setState(() {
    ...
    return WillPopScope(
        ...
        child: Scaffold(
            appBar: AppBar(
              toolbarHeight: 40,
              elevation: 0,
              title: Text('Edit Product', style: GoogleFonts.poppins(fontSize: 16),),
