...
import 'add_screens/add_receipt.dart';
...
class _RecieptScreenState extends State<RecieptScreen> {
  ...
  void setscreenposition() async{
    ...
    screen.setString("currentscreen", "recieptscreen");
  ...
  void delete (String id) async{
    try{
      var rsp = await apiurl("/member/process", "customer_receipt.php", {
        "type": "delete",
        "id": id.toString(),
      });
      ...
      if(rsp.containsKey('status')){
        ...
        showPrintedMessage(context, "Failed", rsp['error'].toString().replaceAll('_'," "), Colors.white,Colors.redAccent, Icons.info, true, "top");
  ...
  Future getdirectory() async {
    ...
    if(isPermissionStatusGranted){
      final Directory _appDocDirFolder = Directory('${dir!.path}/$Appname/receipts');
      if(await _appDocDirFolder.exists()){
        //debugPrint('in function 1');
        //debugPrint('exists');
        appDocDir = await _getDownloadDirectory();
        final String dirPath = '${appDocDir!.path}/$Appname/receipts';
            ...
            children: [
              ...
              if(showalertdetail==false)
                Positioned(
                  ...
                  left: 0,
                  right: 0,
                  child: BottomBar(lastscreen: "recieptscreen",),
                ),
              if(showalertdetail==false)
                Positioned(
                  top: 100,
